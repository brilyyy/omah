...
> = {
  ...
  file: {
    ...
    hint: "Skip when the file exists",
  },
  dir: {
    label: "Dir exists",
...
function DotfileFormDialog({ mode, dotfile, dotIndex, onClose }: DotfileFormDialogProps) {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const symlinkAction = mode === "edit" && oldSymlink !== value.symlink ? value.symlink : null;
                    ...
                    {field.state.value.map((_step, i) => (
                      <div key={i} className="rounded-lg border border-border p-3 space-y-2.5">
                        ...
                        <div className="space-y-1">
                          ...
                          <form.Subscribe
                            selector={(s) =>
                              (s.values.setup[i] as (typeof s.values.setup)[number])?.checkType ??
                              "none"
                            }
                          >
                            {(checkType) => (
...
function FieldError({ field }: { field: any }) {
  ...
  const message = typeof first === "string" ? first : ((first as any)?.message ?? String(first));
